...
/**
 * Extracts the target and parent CSS selectors from a ruleset's showingMatcher detector.
 * Handles both array and object structures generated by the LLM safely.
 *
 * @param {Object} ruleset - The complete Consent-O-Matic ruleset object
 * @returns {Object} - Contains matcherTargetSelector (string|null) and matcherParentSelector (string|null)
 */
function findShowingMatcherSelector(ruleset) {
    let matcherTargetSelector = null;
    let matcherParentSelector = null;
    
    try {
        const showingMatcher = Object.values(ruleset)[0].detectors[0].showingMatcher;
        ...
        console.error("Warning: Could not safely extract showingMatcher from ruleset.", err.message);
    }
 ...
 * Implemented a geometric filter that discards containers smaller than 100x100 pixels to 
 ...
 * Orchestrates the full interaction lifecycle using the provided LLM-generated ruleset configuration.
 *
 * @param {Frame} targetFrame - The specific Puppeteer Frame instance to evaluate
 * @param {Object} ruleset - The complete ruleset schema generated for testing
...
async function runTest(ruleset) {
    ...
    try {
        ...
        const { matcherTargetSelector, matcherParentSelector } = findShowingMatcherSelector(ruleset);
